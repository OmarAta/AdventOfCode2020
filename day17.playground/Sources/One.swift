...
public class One {
    ...
    public init(_ input: String) {
        for x in -15 ... 15 {
            for y in -15 ... 15 {
                for z in -10 ... 10 {
                    map[Position(x: x, y: y, z: z)] = Cube(active: false)
                }
            }
        }
        ...
        for (y, row) in rows.enumerated() {
            ...
            for (x, cube) in cubes.enumerated() {
                map[Position(x: x, y: y, z: 0)] = Cube(active: (cube == "#"))
            }
        }
    ...
    private func countActiveNeighbors(_ position: Position) -> Int {
        ...
        for x in -1 ... 1 {
            for y in -1 ... 1 {
                for z in -1 ... 1 {
                    if !(x == 0 && y == 0 && z == 0) {
                        if let cube = map[Position(x: position.x + x, y: position.y + y, z: position.z + z)] {
