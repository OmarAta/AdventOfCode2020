        ...
        if let byr = passport.byr,
           ...
           let pid = passport.pid {
            if ((byr >= 1920 && byr <= 2002) &&
                (iyr >= 2010 && iyr <= 2020) &&
                (eyr >= 2020 && eyr <= 2030) &&
                ((hgt.unit == "cm" && hgt.value >= 150 && hgt.value <= 193) || (hgt.unit == "in" && hgt.value >= 59 && hgt.value <= 76)) &&
                (["amb","blu","brn","gry","grn","hzl","oth"].contains(ecl)) &&
                (hcl.starts(with: "#") && hcl.filter(\.isHexDigit).count == 6 && hcl.count == 7) &&
                (pid.count == 9 && Int(pid) != nil)
            ) {
                passport.valid = true
            }
        }
